...
<div class="portlet box blue">
	...
	<div class="table-responsive">
		<form class="form-horizontal" role="form" id="formid" action="${base}/per/perbadinfo" method="get">
		...
		<table  class="table table-bordered table-hover">
			...
			<tbody>
				[#list page.content as perbadinfo]
				<tr>
					<td  style="text-align:center;">${pageable.offset + perbadinfo_index+1}</td>
					<td  style="text-align:left;">${perbadinfo.name}</td>
					<td>${perbadinfo.type }</td>
					<td>${perbadinfo.title }</td>
					<td>${perbadinfo.source }</td>
					<td>${perbadinfo.issuedate} </td>
 					<td><a class="btn btn-xs btn-success" href="${base}/per/perbadinfo/detail/${perbadinfo.id}">查看</a></td>
				</tr>
				[/#list]
			</tbody>
		</table>
		[#assign pagepath="${base}/corp/perbadinfo?name=${name}&type=${type}&source=${source}&"]  
