...
[@layout.main modelcode=['collectlog','datacollect'] otherjs=jss othercss=css sername=['数据采集','采集日志']  ]
[#assign  tablewidth=(280+datelist?size*110)+'px']


<form class="form-horizontal" role="form" id="formid" action="${base}/urlmgr/collectlog" method="get"  autocomplete="off">
<div class="col-xs-12" >
<table class="table-form"  > 
	...
	[#if searchtype=="oneweek"]近一周数据采集量统计 
	[#elseif searchtype=="onemonth"]近一个月数据采集量统计 
	[#elseif searchtype=="thisyear"]今年数据采集量统计 
	[#elseif searchtype=="lastyear"]去年数据采集量统计 
	[#elseif searchtype=="subtime"]
		[#if searchtypesub=="day"]
		${(begindate?string('yyyy-MM-dd'))!}~${(enddate?string('yyyy-MM-dd'))!}数据采集量按日统计
		[#elseif searchtypesub=="month"]
		${(begindate?string('yyyy-MM'))!}~${(enddate?string('yyyy-MM'))!}数据采集量按月统计
		[#elseif searchtypesub=="year"]
		${(begindate?string('yyyy'))!}~${(enddate?string('yyyy'))!}数据采集量按年统计
		[/#if]
	[/#if]
