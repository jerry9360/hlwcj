...
[@layout.main modelcode=['logsave','datastore'] otherjs=jss othercss=css sername=['数据存储','存储日志']  ]
[#assign  tablewidth=(280+datelist?size*110)+'px']

<form class="form-horizontal" role="form" id="formid" action="${base}/datamgr/logsave" method="get"  autocomplete="off">
<div class="col-xs-12" >
<table class="table-form"  > 
	...
	<div class="col-xs-12" style="text-align:center;font-size:16px;font-weight: bold;padding-bottom: 10px;margin-top:10px;">
		...
		[#if searchtype=="oneweek"]近一周数据存储量统计 
	[#elseif searchtype=="onemonth"]近一个月数据存储量统计 
	[#elseif searchtype=="thisyear"]今年数据存储量统计 
	[#elseif searchtype=="lastyear"]去年数据存储量统计 
	[#elseif searchtype=="subtime"]
		[#if searchtypesub=="day"]
		${(begindate?string('yyyy-MM-dd'))!}~${(enddate?string('yyyy-MM-dd'))!}数据存储量按日统计
		[#elseif searchtypesub=="month"]
		${(begindate?string('yyyy-MM'))!}~${(enddate?string('yyyy-MM'))!}数据存储量按月统计
		[#elseif searchtypesub=="year"]
		${(begindate?string('yyyy'))!}~${(enddate?string('yyyy'))!}数据存储量按年统计
		[/#if]
	[/#if]
