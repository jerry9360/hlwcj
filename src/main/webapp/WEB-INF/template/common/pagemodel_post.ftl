<div id="pagination" class="row">
	...
	<div class="col-md-7 col-sm-12">
		<div class="dataTables_paginate paging_simple_numbers" id="sample_2_paginate" style="float:right;margin:0;">
			<ul class="pagination">
				<li class="prev"><a href="javascript:void(0)" onclick="searchpage('0')">首页</a></li>
				[#if page.hasPrevious()]
					<li tabindex="0" class="paginate_button previous" id="sample_2_previous" aria-controls="sample_2"><a href="javascript:void(0)" onclick="searchpage('${page.number - 1}')">上一页</a></li>
				[#else]
					...
					[#if pageable.offset/page.size>1 && pageable.offset/page.size<pages -2]
						[#list pageable.offset/page.size-2.. pageable.offset/page.size+2 as m]
							<li tabindex="0" class="paginate_button [#if pageable.offset/page.size =m]active[/#if]" aria-controls="sample_2">
								<a href="javascript:void(0)" onclick="searchpage('${m}')">${m+1}</a>
							</li>
						[/#list]
					[/#if]
				...
				[#if page.hasNext()]
					<li tabindex="0" class="paginate_button next" id="sample_2_next" aria-controls="sample_2"><a href="javascript:void(0)" onclick="searchpage('${page.number + 1}')">下一页</a></li>
                [#else]
                ...
                [/#if]
                [#if page.totalElements%page.size=0]
                 	<li class="next"><a href="javascript:void(0)" onclick="searchpage('${(page.totalElements/page.size)?int-1}')">末页</a></li>
				[#else]
					 <li class="next"><a href="javascript:void(0)" onclick="searchpage('${(page.totalElements/page.size)?int}')">末页</a></li>
				[/#if]	    
			</ul>
		</div>
	</div>
 ...
 <div style="display:none;">
 	<form id="pageform" action="${pagepath}" method="post">
 		[#if pageparams??]
 			[#list pageparams as param]
 				<input type="hidden" name="${param[0]}" value="${param[1]}"></input>
 			[/#list]
 		[/#if]
 		[#if (pageable.sort)??]
 			[#list pageable.sort.iterator() as order]
 				<input type="hidden" name="sort" value="${order.property},${order.direction}"></input>
 			[/#list]
 		[/#if]
 ...
 <script type="text/javascript">
	function searchpage(position){
		$("#pageform").children("input[name='page']").val(position);
		$("#pageform").submit();
	}
 </script>