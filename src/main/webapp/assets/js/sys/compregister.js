		...
		form.validate({
			...
			messages : {
				loginname : {
					...
					minlength : jQuery.validator.format("用户名长度不小于6位！"),
				...
				companyname : {
					required : jQuery.validator.format("请输入公司名称！"),
					maxlength : jQuery.validator.format("公司名称长度过长！")
				},
				linkman : {
					required : jQuery.validator.format("请输入公司联系人！"),
					maxlength : jQuery.validator.format("公司联系人名称过长！")
				},
				address : {
					required : jQuery.validator.format("请输入公司地址！"),
					maxlength : jQuery.validator.format("公司地址名称长度过长！")
				},
				companycode : {
					required : jQuery.validator.format("请输入公司机构代码！"),
					maxlength : jQuery.validator.format("公司机构代码长度过长！")
				},
				mail : {
				...
				llinkphone : {
					required : jQuery.validator.format("请输入电话号码！"),
					number : jQuery.validator.format("格式有误！不需要'-'")
				},
				memo : {
					maxlength : jQuery.validator.format("注册说明信息太长！"),
				}
			},
...
function checkLoginname(loginname){
	...
	$.ajax({
		type:"POST",
		url:base + "/common/compregisterinfo/checkname",
...
function checkPhone(phone){
	...
	$.ajax({
		type:"POST",
		url:base + "/common/compregisterinfo/checkphone",
		data:{phone:phone},
		async:false,
		success:function(data){
			if(data=="exist"){
					...
					function(file, response) {
						...
						if (typeof (info.state) != 'undefined') {
							if (info.state == 'SUCCESS') {
								...
								var	html =  "<span><input type='hidden' name='compregisterannex[0].ucode' value='"
											+ ucode
											+ "'>"
											+ "<input type='hidden' name='compregisterannex[0].fileid' value='"
											+ id
											+ "'>"
											+ "<input type='hidden' name='compregisterannex[0].annexname' value='身份证'></span>"
											+ "<div><image  name='fujian_list' src='"
											+ fileAddr
											+ "'style='height:200px; margin-bottom: 0px; margin-left: 75px'/>" 
											+ "</div>";
								$("#personIdList").html(html);
								$("#dpi").show();
							} else {
					...
					function(file, response) {
						...
						if (typeof (info.state) != 'undefined') {
							if (info.state == 'SUCCESS') {
								...
								var fileAddr = info['url'];
								var ucode = $("#setUcode").val();
								var	html =  "<span><input type='hidden' name='compregisterannex[1].ucode' value='"
											+ ucode
											+ "'>"
											+ "<input type='hidden' name='compregisterannex[1].fileid' value='"
											+ id
											+ "'>"
											+ "<input type='hidden' name='compregisterannex[1].annexname' value='船公司营业执照'></span>"
											+ "<div><image name='fujian_list' src='"
											+ fileAddr
											+ "'style='height:200px; margin-bottom: 0px; margin-left: 75px'/>" 
											+ "</div>";
