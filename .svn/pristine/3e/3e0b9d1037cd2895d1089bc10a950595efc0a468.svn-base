...
[@layout.main modelcode=['fw_mgr' ] otherjs=jss othercss=css sername=['数据服务','接口管理']  ]


[#include "interfacemanager/tanchu.ftl"]
...
<button class="btn btn-sm btn-primary" type="button" style="margin-left:15px;" onclick="adddinfo()" >新 增</button>
<table class="table table-bordered table-hover" style="margin-top:10px;">
			...
			[#list serviceinterface as serviceinterface ]
				<tr>
					...
					<a class="btn btn-xs btn-primary"  onclick="editform('${serviceinterface.id}')"   >编辑</a>
					<a class="btn btn-xs btn-danger"   onclick="deleteinfo('${serviceinterface.id}')"   >删除</a>
					</td>						
				</tr>
			[/#list]
	 ...
	 function select(){
		...
		/*  alert(ine); */
		 location.href=base+'/servicemgr/listmgr?interface_name='+ine;
