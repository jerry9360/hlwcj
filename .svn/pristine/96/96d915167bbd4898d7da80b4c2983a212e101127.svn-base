...
[@layout.main modelcode=['wash_dishonestman','infowash'] otherjs=jss sername=['数据清洗','失信被执行人'] pagename='失信被执行人']
		<form class="form-horizontal" role="form" id="formid" action="${base}/infowash/dishonestman" method="get">
		...
		<table  class="table table-bordered table-hover">
			...
			<tbody>
				...
				<tr>
					...
					<td  style="text-align:left;">${dishonestMan.corp_name}</td>
					<td>
						...
						[#elseif dishonestMan.industry=='4']
							道路运输市场
						[/#if]
					</td>					<td>${dishonestMan.name }</td>
					<td>${dishonestMan.source }</td>
					<td>${dishonestMan.releasetime}</td>
 					<td><a  class="btn btn-xs btn-primary" href="${base}/infowash/dishonestman/${dishonestMan.id}">处 理</a>
		...
		[#assign pagepath="${base}/infowash/dishonestman?corp_name=${corp_name}&industry=${industry}&"]  
